/*
 *   STUDENT NAME      :  Haoran Wang
 *   STUDENT ID        :  260727267
 *   
...
//		}
//		MyBigInteger remainder=this.clone();
//		MyBigInteger partD=new MyBigInteger(base);
//		MyBigInteger quotient = new MyBigInteger(0,base);
//		MyBigInteger quotientPartial = new MyBigInteger(base);
//		while (remainder.compareTo(divisor) >=0) {
//			int j = remainder.coefficients.size()-1;
//			while (partD.compareTo(divisor)<0) {
//				partD.coefficients.add(0,remainder.coefficients.get(j));
//				j=j-1;
//			}
//
//			quotientPartial = partD.slowdividedBy(divisor);
//			while(j>-1){
//				quotientPartial.coefficients.add(0,0);
//				j=j-1;
//			}
//
//			quotient = quotient.plus(quotientPartial);
//			remainder = remainder.minus(divisor.times(quotientPartial));
//			partD=new MyBigInteger(base);
//		}
//		return quotient;
//	}
		...
		while (remainder.compareTo(divisor) == 1){
			index++;
			remainder.coefficients.remove(0);
		}
      ...
      if(this.compareTo(divisor) ==0){
		...
		while(index > 0){
			...
			if(remainder.compareTo(divisor) <0 && index >0){
				index--;
				remainder.coefficients.add(0,this.coefficients.get(index));
			...
			while(remainder.compareTo(divisor) < 0 && index > 0){
				index--;
				remainder.coefficients.add(0,this.coefficients.get(index));
				//delete leading zero
				while(remainder.coefficients.size()>0&&remainder.coefficients.get(remainder.coefficients.size()-1)==0){
				...
				int intget=remainder.slowdividedBy(divisor).coefficients.get(0);
			//System.out.println("intget is: "+intget);
			quotient.coefficients.add(0,intget);
			//remainder=remainder.minus(divisor.timesSingleDigit(quotient.coefficients.get(0)));
			//delete leadingzero
			while(remainder.coefficients.size()>0&&remainder.coefficients.get(remainder.coefficients.size()-1)==0){
				remainder.coefficients.remove(remainder.coefficients.size()-1);
			}
			...
			if(remainder.compareTo(divisor)<0){break;}
			remainder=remainder.minus(divisor.timesSingleDigit(quotient.coefficients.get(0)));
		}
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		//1.convert remainder to base 10 number
//		int sum = 0;
//		for(int i = 0; i < remainder.coefficients.size();++i)
//		{
//			sum += remainder.coefficients.get(i)* Math.pow(remainder.base,i);
//                   num1                        num2



	    MyBigInteger base10Num = new MyBigInteger(0, 10);
		MyBigInteger num2 = new MyBigInteger(1    , 10);

		for (int i = 0; i < remainder.coefficients.size(); ++i) {
		    MyBigInteger num1 = new MyBigInteger(remainder.coefficients.get(i), 10);

			MyBigInteger addedNum = num1.times(num2);
			//num2=num2.times(new MyBigInteger(remainder.base  , 10));
			base10Num = base10Num.plus(addedNum);
			num2=num2.times(new MyBigInteger(remainder.base  , 10));
		}

		//2.convert that base 10 number to newBase number
		//以从十进制的9转到二进制的8为例子 当8大于0，存 0 0 1

		MyBigInteger divisor = new MyBigInteger(newBase,10);
		//System.out.println(base10Num.mod(divisor).coefficients.toString());

		while(base10Num.compareTo(new MyBigInteger(0,10)) == 1){

			//System.out.println("result is: "+result.toString());
			result.coefficients.add(base10Num.mod(divisor).coefficients.get(0));
			//System.out.println("result after is: "+result.toString());
			base10Num=base10Num.dividedBy(divisor);
		}
	          return result;
	     }
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		ArrayList<MyBigInteger>  factors = new ArrayList<MyBigInteger>();
			...
			////找到范围 2-sqrt（m）内所有质数依次去找 例如 2，3，5，7 i方小于等于m
		MyBigInteger start = new MyBigInteger( 2 , base);
		MyBigInteger zero = new MyBigInteger(0,base);
		MyBigInteger one = new MyBigInteger(1,base);
        MyBigInteger kl =this.clone();



		for(MyBigInteger i=start ; (i.times(i)).compareTo(kl)!=1;i=i.plus(new MyBigInteger(1,base)))
		{
			//如果this能被i整除并且this不是1的话
			while(kl.mod(i).compareTo(zero)==0 && kl.compareTo(one)!= 0 )
			{
				kl=kl.dividedBy(i);
				factors.add(i);
			}
		}
		//如果this还不等于1，add this to factors
		if(kl.compareTo(new MyBigInteger(1,base)) != 0){
			factors.add(kl);
		}
